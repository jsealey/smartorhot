class ExtendedProfile < ActiveRecord::Base
  ...
  has_attached_file :avatar, :storage => :dropbox, :dropbox_credentials => "#{Rails.root}/config/dropbox.yml", :styles => { :medium => "300x300>", :thumb => "100x100>" }, :dropbox_options => {:unique_filename => true,:path => proc { |style| "#{style}/#{id}_#{avatar.original_filename}"}}
